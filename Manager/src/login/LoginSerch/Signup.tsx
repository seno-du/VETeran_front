...
export default function Signup() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // ✅ 과 선택 시 숫자로 변환
    if (name === "managerLicenseNum") {
      updatedValue = departmentMapping[value] || value;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("http://localhost:80/api/Login/join", {
        method: "POST",
      ...
      if (response.ok && data.result === "OK") {
        ...
        // ✅ 백엔드에서 JWT 반환 여부 확인 (예시로 추가했지만, 실제 반환하는지 체크 필요!)
        if (data.accessToken) {
